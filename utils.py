...
class RealEstateDataManager:
    ...
    def _load_all_data(self):
        """Load all data files"""
        self.properties = self._load_json_file(os.path.join(self.data_dir, "properties", "active_listings.json"))
        self.agents = self._load_json_file(os.path.join(self.data_dir, "agents", "agent_profiles.json"))
        self.market = self._load_json_file(os.path.join(self.data_dir, "market", "market_analytics.json"))
        self.clients = self._load_json_file(os.path.join(self.data_dir, "clients", "client_database.json"))
        self.amenities = self._load_json_file(os.path.join(self.data_dir, "amenities", "local_amenities.json"))
        self.transactions = self._load_json_file(os.path.join(self.data_dir, "transactions", "recent_sales.json"))
        self.areas = self._load_json_file(os.path.join(self.data_dir, "areas", "city_overview.json"))
    ...
    def _matches_filter(self, prop: Dict[str, Any], filters: PropertyFilter) -> bool:
        ...
        if filters.min_bedrooms and prop.get("bedrooms", 0) < filters.min_bedrooms:
            return False
        if filters.max_bedrooms and prop.get("bedrooms", float('inf')) > filters.max_bedrooms:
            return False
        
        # Bathrooms
        if filters.min_bathrooms and prop.get("bathrooms", 0) < filters.min_bathrooms:
            return False
        if filters.max_bathrooms and prop.get("bathrooms", float('inf')) > filters.max_bathrooms:
            return False
        ...
        # Property types
        if filters.property_types and prop.get("property_type") not in filters.property_types:
            return False
        
        # Square footage
        if filters.min_sqft and prop.get("square_feet", 0) < filters.min_sqft:
            return False
        if filters.max_sqft and prop.get("square_feet", float('inf')) > filters.max_sqft:
            return False
        ...
        if filters.features:
            ...
            for required_feature in filters.features:
                if not any(required_feature.lower() in feature.lower() for feature in prop_features):
                    return False
    ...
    def get_properties_by_area(self, area: str) -> List[Dict[str, Any]]:
        ...
        properties = self.get_all_properties()
        return [prop for prop in properties if prop.get("area", "").lower() == area.lower()]
    ...
    def get_agent_performance(self, agent_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "client_rating": self._calculate_avg_rating(agent.get("client_testimonials", []))
        }
    ...
    def get_schools_by_area(self, area: str) -> List[Dict[str, Any]]:
        ...
        for school_type in ["elementary_schools", "middle_schools", "high_schools", "private_schools"]:
    ...
    def get_area_amenities(self, area: str) -> Dict[str, Any]:
        ...
        amenities["parks"] = [park for park in parks if park.get("area", "").lower() == area.lower()]
        ...
        for shop_type, shops in shopping.items():
            if isinstance(shops, list):
                area_shopping[shop_type] = [shop for shop in shops if shop.get("area", "").lower() == area.lower()]
    ...
    def calculate_market_trends(self, area: str = None) -> Dict[str, Any]:
        ...
        avg_days_on_market = sum(sale.get("days_on_market", 0) for sale in sales) / total_sales
        avg_price_per_sqft = sum(sale.get("price_per_sqft", 0) for sale in sales) / total_sales
